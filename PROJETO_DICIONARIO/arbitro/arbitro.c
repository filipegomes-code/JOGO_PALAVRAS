...
void warnsAll(const char* mensagem) {
    ...
    for (int i = 0; i < cont_players; i++) {
        ...
        if (hPipeComando != INVALID_HANDLE_VALUE) {
            DWORD byteswrite;
            BOOL wrote = WriteFile(hPipeComando, mensagem, (DWORD)(strlen(mensagem) + 1), &byteswrite, NULL);
            if (!wrote) {
...
void removePlayer(const char* nome) {
    ...
    for (int i = 0; i < cont_players; i++) {
        if (strcmp(PLAYER_NAMES[i], nome) == 0) {
            ...
            if (hPipeComando != INVALID_HANDLE_VALUE) {
                DWORD byteswrite;
                WriteFile(hPipeComando, TIPO_EXCLUIR, (DWORD) (strlen(TIPO_EXCLUIR) + 1), &byteswrite, NULL);
                CloseHandle(hPipeComando);
            }
            else {
...
void player_rejeitado(HANDLE hpipe, const char* TIPO) {
    ...
    DWORD byteswrite;
    WriteFile(hpipe, TIPO_LIM_PLAYERS, (DWORD)(strlen(TIPO_LIM_PLAYERS) + 1), &byteswrite, NULL);
...
void Comandos(mensagem msg, HANDLE hpipe) {
    if (strcmp(msg.tipo, TIPO_ENTRAR) == 0) {
        ...
        DWORD bytesWritten;
        WriteFile(hpipe, "aceite", (DWORD)(strlen("aceite") + 1), &bytesWritten, NULL);
        ...
        DWORD byteswrite;
        WriteFile(hpipe, lista,(DWORD) (strlen(lista) + 1), &byteswrite, NULL);
        ...
        ReleaseMutex(hmutex);
        DWORD byteswrite;
        WriteFile(hpipe, pontos, (DWORD)(strlen(pontos) + 1), &byteswrite, NULL);
        CloseHandle(hpipe);
        // warnsall (opcional)
    }
...
void anunciar_vencedor_encerrar() {
    ...
    for (int i = 0; i < cont_players; i++) {
        ...
        if (hPipeComando != INVALID_HANDLE_VALUE) {
            DWORD byteswrite;
            WriteFile(hPipeComando, TIPO_ENCERRAR, (DWORD)(strlen(TIPO_ENCERRAR) + 1), &byteswrite, NULL);
            CloseHandle(hPipeComando);
        }
        else {
...
int valida_pal(const char* username, const char* palavra, HANDLE hpipe) {
    ...
    if (!pal_existe(palavra)) {
        ...
        DWORD write;
        WriteFile(hpipe, resposta, (DWORD)(strlen(resposta) + 1), &write, NULL);
